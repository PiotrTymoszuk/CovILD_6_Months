...
# Serial testing of the prediction quality with multiple random training/test splits -----
  ...
  test_accuracy <- function(boot_split_lst, outcome, pred_fun = 'knn', 
                            generate_random = F, ci_method = 'percentile', 
                            .parallel = T, ...) {
    ...
    ## and bca provided by coxed package
    ...
    summary_qc <- qc_results %>% ## there are settings (TP = 0), where Se/Sp cannot be computed
      filter(!is.na(Se), 
             !is.na(Sp)) %>% 
      map_dfr(function(x) c(mean(x), 
                            ci_fun(x)) %>% 
